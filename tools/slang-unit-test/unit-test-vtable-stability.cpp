...
// ---------------------------------------------------------------------------
// IModulePrecompileService_Experimental : ISlangUnknown  (own slots 3-6)
// ---------------------------------------------------------------------------
struct IModulePrecompileServiceProbe : IModulePrecompileService_Experimental
{
    int lastSlot = -1;
    SLANG_NO_THROW SlangResult SLANG_MCALL queryInterface(SlangUUID const&, void**) SLANG_OVERRIDE
    {
    ...
    SLANG_NO_THROW SlangResult SLANG_MCALL precompileForTarget(SlangCompileTarget, ISlangBlob**)
        SLANG_OVERRIDE
    {
        lastSlot = 3;
        return SLANG_OK;
    ...
    SLANG_NO_THROW SlangResult SLANG_MCALL
    getPrecompiledTargetCode(SlangCompileTarget, IBlob**, IBlob**) SLANG_OVERRIDE
    {
        lastSlot = 4;
        return SLANG_OK;
    }
    ...
    SLANG_NO_THROW SlangResult SLANG_MCALL getModuleDependency(SlangInt, IModule**, IBlob**)
        SLANG_OVERRIDE
    {
        lastSlot = 6;
        return SLANG_OK;
...
SLANG_UNIT_TEST(vtableIModulePrecompileService)
{
    IModulePrecompileServiceProbe p;
    callSlot(&p, 3);
    SLANG_CHECK(p.lastSlot == 3); // precompileForTarget
    callSlot(&p, 4);
    SLANG_CHECK(p.lastSlot == 4); // getPrecompiledTargetCode
    callSlot(&p, 5);
    SLANG_CHECK(p.lastSlot == 5); // getModuleDependencyCount
    callSlot(&p, 6);
    SLANG_CHECK(p.lastSlot == 6); // getModuleDependency
}
