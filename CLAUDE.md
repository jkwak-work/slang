...
- Extending an interface by adding methods at the end is non-breaking for existing callers
  (their vtable pointers still resolve correctly), but the interface UUID must be updated if
  the interface uses one.
