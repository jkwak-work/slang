...
| `SLANG_DXC_BUILD_FROM_SOURCE`         | Unset (source build on macOS; auto on native Linux x86_64) | `ON`: build DXC from source on Windows, Linux, and macOS; `OFF`: use prebuilt when available; unset: build from source on macOS and auto-select on native Linux x86_64 (see [DXC GLIBC auto-detection](#dxc-glibc-auto-detection)) |
